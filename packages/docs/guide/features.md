...
Built-in AI assistant accessible via the AI tab in the properties panel or <kbd>⌘</kbd><kbd>J</kbd>. Communicates directly with model providers from the app — curated OpenRouter models, direct OpenAI Codex, and experimental desktop Claude Code/Codex CLI backends. No proxy backend is required for the direct providers. API keys are stored locally (localStorage fallback in browser). In the desktop app, the local CLI backends register against a loopback bridge, stream response text and completed tool calls back into the same chat timeline, call OpenPencil tools through a staged helper script, support JSX `render` from a source workspace with `esbuild` installed, and can be stopped mid-run from the chat UI.

**Model selector** with curated models: Claude, Gemini, GPT, DeepSeek, Qwen, Kimi, Llama — stored in `@open-pencil/core` constants with benchmark-ranked tags. The chat composer also supports local file attachments, pasted images for multimodal prompts, locally restored conversation history, locally persisted prompt recall with ↑/↓, and a New session reset for native Claude Code/Codex CLI conversations. Responses stream as markdown (vue-stream-markdown).
